...
using Xunit;
using Depot = ApiTestingFramework.ResourceObjects.Depot;
...
namespace ApiTestingFramework.StepDefinitions
{
    ...
    public class DepotStepDefinition :Steps
    {
        ...
        public Task ThenIReceiveAPIResponsefromkafkaserviceAsync()
        {
            ...
            Base.Instance.testData.AddResponseJson(httpResponseMessage);
        ...
        {
            var responseJson = Base.Instance.testData.GetResponseJson().ToString();
        ...
        {
            Assert.Equal(200, Base.Instance.testData.GetResponseStatusCode());
            var responseJson = Base.Instance.testData.GetResponseJson().ToString();
            ODataResponse<DepotODataViewModel>? response = JsonConvert.DeserializeObject<ODataResponse<DepotODataViewModel>>(responseJson);
        ...
        {
            Base.Instance.testData.GetResponseJson();
        }

        [Then(@"The Status Code should be (.*)")]
        public void ThenTheStatusCodeShouldBe(int statuscode)
        {
            Assert.Equal(statuscode, Base.Instance.testData.GetResponseStatusCode());
        }
